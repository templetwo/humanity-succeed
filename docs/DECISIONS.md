...
### WP3 evaluator commissioning (opened 2026-09-26 by Anthony; MacBook seat, claude-opus-5-5)

Anthony opened WP3 on 2026-09-26 ("you open wp3"); Grok red-teams the WP2 compiler and observation
boundary separately. The design is in `docs/WP3_DESIGN.md`, and the build ran as four verified
stages. Stage 0 froze WP0–WP2 behaviour. Stage 1 was the lead's contract. Stages 2 and 3 were built
by Sonnet builders on disjoint files, after a Sonnet advisory pass, and each item was adversarially
verified. Stage 4 was the lead's run and receipts. **Model called: no. Model trained: no.**

| ID | Decision | Reason / boundary |
|---|---|---|
| B48 | **No holdback custodian has been named.** All 160 builder-constructed trajectories are split `commissioning_dev` (`cases/commissioning_suite_v1/`). The partitioner designates 40 of them as `holdback_designate`, proves 120/40 with no group split, and reports them as builder-authored, exposed, and not independent. Formal commissioning needs a custodian-supplied suite with these properties: 40 trajectories, split `commissioning_holdback`, stored outside the repository and the state root, sealed by a `CustodyRecord` hash (`scripts/seal_holdback.py`, `docs/HOLDBACK_CUSTODY.md`), and passing the split audit against the development suite. An exposure ledger refuses a second formal run on the same holdback (A20) | §5.2: without a custodian, `independent_holdback=false`. §9: "The agent cannot author, inspect, and then call its own fixtures independently held back." Custodian identity is attested by the record, not verified |
| B49 | **WP3 changes no evaluator or engine behaviour.** All seven classes are expressed with registered predicates; the evaluator stays `hs-evaluator/0.2.0`. `tests/golden/` freezes WP0–WP2 evaluations, event streams and compiler output at stage 0 | Commissioning an evaluator while editing it would commission a moving target |
| B50 | **Golden expectations are specification.** Expectations are written from the class contracts before any run. The generator code and tests check only enactment (what a trajectory did) and never run the evaluator. A miss is triaged as either a fixture defect (enactment shows it) or an evaluator finding; it is never fixed by editing the expectation | Expectations measured from the evaluator would make commissioning circular |
| B51 | The suite lives beside `cases/commissioning_dev/`, not inside it. It is generated byte-for-byte by `scripts/generate_commissioning_suite.py`, and a `--check` test keeps it matching the generators | `load_cases` is recursive. 108 suite cases inside the dev corpus would enter the WP1 compile and the frozen digests |
| B52 | **Controlled mutations:** `tone_shift` (verdict unchanged), `drop_effects` (a pass becomes a non-pass), `notice_first` (under `notification_after_state`, a pass becomes a non-pass), `truncate_finish` (`not_evaluable`, missingness) | These check the evaluator's sensitivity in the directions the spec cares about. The lifecycle requires every applicable invariant to hold |
| B53 | **The lifecycle ceiling at this checkpoint is `mechanically_validated`.** `instrument_commissioned` needs formal = passed and completed semantic review. Semantic commissioning is `pending_no_human_reviews`: 36 judgment-heavy fixtures (the hybrid-scored passes in C1, C2 and C4) need two independent human reviewers, and review import is WP4. The agreement statistics (raw agreement, confusion matrix, prevalence, Cohen's kappa) are implemented and tested on synthetic ratings only | §9: no human reviewers means mechanical commissioning may pass while semantic commissioning stays pending |
| B54 | **First commissioning run (development mode), `8a24ad1`.** 160 of 160 fixtures met their written expectations; all 160 bundles verify and replay as reproduced; all 416 applicable mutation invariants held. Evaluator-sabotage controls: 4 of 4 broken evaluators detected (`docs/receipts/wp3/03_sabotage.stdout`). **Coverage findings:** (F1) the B42 notice-ordering obligation is exercised only by the `notice_first` mutation. No constructed fixture's verdict depends on it: a sabotaged `notification_after_state` misses 0 fixtures and trips 36 mutation invariants. (F2) `feature_preserved` is load-bearing in only 4 fixtures (C7). (F3) The contract's formal statuses `blocked_holdback_exposed` and `blocked_custody_invalid` are never written into a report, because those cases are refused before a run (exit 3). None of these was patched after the run | §9: report exactly which fixtures passed; no population error bound. F1 and F2 are gaps in the suite design (the lead's), recorded rather than fixed after seeing a result. A suite v2 that adds them would be a new, versioned suite |
